...
export const GcsProvider: React.FC<{ children: React.ReactNode }> = ({ children }) => {
  ...
  useEffect(() => {
    fetch(`${API_BASE}/api/system/startup-state`)
      .then(res => res.json())
      .then(data => {
        if (data) {
          if (data.telemetry) {
  ...
  useEffect(() => {
    ...
    const connectWS = () => {
      try {
        ...
        ws.onopen = () => {
          console.log(`[Main Dashboard] Connected to Main Backend Gateway (${WS_BASE}/stream)`);
        };
  ...
  useEffect(() => {
    fetch(`${API_BASE}/api/faults?active=true`)
      .then(res => res.json())
      .then(data => {
  ...
  const clearFault = useCallback((faultId: string) => {
    ...
    fetch(`${API_BASE}/api/faults/${faultId}/remove`, {
      method: 'PATCH',
      headers: { 'Content-Type': 'application/json' },
